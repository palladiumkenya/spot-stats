...
import { ReInitializeSummariesCommand } from '../re-initialize-summaries-command';

@CommandHandler(ReInitializeSummariesCommand)
export class ReInitializeSummariesHandler
  implements ICommandHandler<ReInitializeSummariesCommand> {
  constructor(
  ...
  async execute(command: ReInitializeSummariesCommand): Promise<any> {
    let facility = command.facility;
